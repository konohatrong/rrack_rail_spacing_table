...
import structural
import wind_load
import report  # Calls the updated report.py
...
if st.button("🚀 Run Analysis"):
    ...
    for z in zones:
        ...
        rxn = np.max(np.abs(fem['reactions']))
        mom = fem['max_moment']
        shr = np.max(np.abs(fem['shear']))
    ...
    with c1:
        ...
        st.write(f"**Vdes:** {v_des:.2f} m/s (Md={md}, Mz,cat={mz_cat:.2f})")
    with c2:
    ...
    c1, c2 = st.columns([1, 1])
    with c1:
        st.write(f"**Wind 0°:** Cpe={res0['cpe']:.2f}")
        st.write(f"**Wind 90°:** Cpe={res90['cpe']:.2f}")
        st.info(f"**Governing:** {gov_case}")
    with c2:
        st.pyplot(plot_panel_load(panel_w, panel_d, orient_key, trib_width))
    ...
    with c1:
        ...
        st.caption(f"At Span: {worst_res['span']:.2f} m")
    with c2:
    ...
    wind_dict = {
        'cpe_0': res0['cpe'], 'ratio_0': r0,
        'cpe_90': res90['cpe'], 'ratio_90': r90,
        'governing_case': gov_case, 'note': note,
