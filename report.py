...
def generate_full_report(inputs, wind_res, struct_res, zone_results, critical_res):
    ...
    # Add Utilization column if missing
    ...
    # Using critical case data for example calculation
    # Formula: p = 0.5 * rho * Vdes^2 * Cfig * Cdyn
    rho_air = 1.2
    c_dyn = 1.0
    
    # Find Kl for critical zone
    crit_kl = 1.0
    for z in zone_results:
    ...
    # Calculate Cfig for critical case
    # Cfig = Cpe * Ka * Kc * Kl * Kp
    c_fig = wind_res['cpe_base'] * wind_res['ka'] * wind_res['kc'] * crit_kl * 1.0
    ...
    3. Aerodynamic Shape Factor (Cfig) & Design Pressure (p)
       ...
       Calculation:
       C_fig = {wind_res['cpe_base']:.2f} (Cpe) * {wind_res['ka']} (Ka) * {wind_res['kc']} (Kc) * {crit_kl} (Kl) * 1.0
             = {c_fig:.3f}
             
       p_design = 0.5 * {rho_air} * ({inputs['v_des']:.2f})^2 * {c_fig:.3f} * {c_dyn}
    ...
    # Step 5: Optimization
    step_optimization = f"""
    5. Span Optimization (Iterative FEM)
       Objective: Find Max Span (L) where M* <= Mn ({struct_res['Mn']:.3f} kNm)
       
       Iterative Search for {critical_res['zone']}:
       - Applied Load (w): {critical_res['load']:.3f} kN/m
       - Configuration: {inputs.get('num_spans', 2)} Continuous Spans
       
       Result:
       - Optimal Span Found: {critical_res['span']:.2f} m
       - Resulting Moment (M*): {critical_res['moment']:.3f} kNm
       
       Utilization Ratio (M*/Mn):
       ({critical_res['moment']:.3f} / {struct_res['Mn']:.3f}) * 100 = { (critical_res['moment']/struct_res['Mn'])*100 :.2f} %
    """
