...
def generate_full_report(inputs, wind_res, struct_res, zone_results, critical_res):
    ...
    2. External Pressure Coefficient (Cpe) Analysis:
       ...
       [Check 1] Wind 0 deg (Normal): h/d={wind_res['ratio_0']:.2f} -> Cpe={wind_res['cpe_0']:.2f}
       [Check 2] Wind 90 deg (Parallel): h/b={wind_res['ratio_90']:.2f} -> Cpe={wind_res['cpe_90']:.2f}
...
-------------------------------------------------------
   ...
   4. STRUCTURE: Continuous Beam with {inputs['num_spans']} spans (Min {inputs['num_spans']+1} supports).
   5. EXCLUSIONS: 
      - Fixing/Screw Pull-out capacity MUST be verified separately.
      - Roof structure capacity is NOT checked.
      - Deflection (SLS) is NOT checked.
